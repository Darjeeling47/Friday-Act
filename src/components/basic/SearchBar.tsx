      ...
      {filter && (
        <>
          ...
          {/* <div className='relative'>
            {
              isFilterOpen && (
                <div className='w-fit absolute bg-neutral-50 px-4 py-1 shadow-2'>
                  {
                    filter.map((item, index) => (
                      <div key={index} className='flex flex-col gap-2 w-fit max-w-80'>
                        <p className='text-md font-light text-neutral-600 w-full'>{item.category}</p>
                        <div className='flex flex-row gap-2 w-full'>
                          {item.options.map((option: any, index: number) => (
                            <button 
                              key={index} 
                              className='inline-block px-4 py-1 rounded-3xl bg-neutral-50 border border-neutral-600'
                              
                            >
                              <p className='text-md font-light text-neutral-600'>{option}</p>
                            </button>
                          ))}
                        </div>
                      </div>
